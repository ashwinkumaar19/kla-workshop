...
with open("Milestone_Input\Milestone_Input\Milestone 6\Source.txt","r") as f:
    lines = f.readlines()
...
target_points = []
n = 0
with open("Milestone_Input\Milestone_Input\Milestone 6\POI.txt","r") as d:
    for i in d:
        if "xy" in i:
            temp = []
            l = i.split()
    ...
    if "xy" in i:
        ...
        if int(l[1]) == n:
            ...
            for j in range(2,len(l)):
                temp.append(int(l[j]))
                points.append(temp)
        if is_match(temp,target_points[0]):
            o.write("boundary\n")
            o.write("layer 1\n")
            o.write("datatype 0\n")
            o.write(i)
